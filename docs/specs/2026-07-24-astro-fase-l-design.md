...
Turno 2 (al TOPE de Interpret, ANTES del chat):
    si pendingCmd != "":
        ...
        si el texto es AFIRMATIVO (sí/dale/confirmo/hacelo/correlo/ok) → correr `sh -c <cmd>` → decir el resultado
        si es NEGATIVO (no/cancelá/dejá/olvidalo) → "Listo, cancelado."
        si es otra cosa → cancelar el pending y procesar el texto como pedido nuevo (default seguro: NO corre)
```

- **La confirmación se resuelve ANTES del LLM** (es un sí/no, no un pedido nuevo): matcheo por keywords sobre
  el texto normalizado (minúsculas, sin acentos — reusa `normalize` de `interpreter.go`). Ante ambigüedad →
  **no corre** (cancela y reprocesa).
...
- **Correr:** `Runner.Run("sh", "-c", <cmd>)` — **acá sí hay shell arbitrario** (necesario para pipes/redirects/
  globs = "todo"), pero **solo después de que vos confirmes**. Reply hablado: si la salida es corta (≤ ~200
  chars) se dice; si es larga → "Listo (la salida quedó en la terminal)"; si falla → "Falló: <error>".
...
- **Backstop catastrófico** (rechazo directo, aunque confirmes): patrones que borran todo o rompen el sistema
  — `rm -rf /`, `rm -rf ~`, `rm -rf /home`, `dd `, `mkfs`, `sudo`, `:(){` (fork bomb), `> /dev/sd`, `shutdown`,
  `reboot`, `chmod -R /`, `chown -R /`. Se chequea sobre el comando normalizado. **Honestidad: NO es airtight**
  (es un backstop contra alucinaciones obvias, no una sandbox); la confirmación sigue siendo la guarda real.
...
~/projects/astro/
  llm.go       — (modificar) LLMConfig.ExecEnabled; campos execEnabled/pendingCmd; confirmación al tope de
                 Interpret; ruteo de "ejecutar"; systemPrompt; helpers isAffirmative/isNegative/isCatastrophic/execAction
